...
from .serializers import CurrencyExchangeSerializer, UserSerializer, UserBalanceSerializer
...
class CurrencyRateView(APIView):
    ...
    @swagger_auto_schema(
        ...
        responses={200: "Exchange rate fetched.", 403: "Zero balance.", 400: "Invalid currency code.", 500: "API error."}
    )
    def post(self, request):
        ...
        response = requests.get(f"https://v6.exchangerate-api.com/v6/{EXCHANGE_API_KEY}/latest/UAH")
        ...
        if not rate:
            return Response({"error": "Wrong currency code"}, status=status.HTTP_400_BAD_REQUEST)

        with transaction.atomic():
            CurrencyExchange.objects.create(user=request.user, currency_code=currency_code, rate=rate)
            user_balance.balance -= 1
            user_balance.save()

        return Response({"currency": currency_code, "rate": rate, "balance": user_balance.balance})
...
class HistoryView(APIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('currency', openapi.IN_QUERY, description="Currency code to filter by (optional)", type=openapi.TYPE_STRING),
            openapi.Parameter('date', openapi.IN_QUERY, description="Date to filter by (optional, format: YYYY-MM-DD)", type=openapi.TYPE_STRING)
        ],
        responses={200: CurrencyExchangeSerializer(many=True), 403: "Authentication failed."}
    )
