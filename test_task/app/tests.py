...
class CurrencyExchangeTests(APITestCase):
    ...
    def test_currency_rate(self, mock_get):
        ...
        response = self.client.post(
            "/currency/", {"currency_code": "USD"}, headers=self.auth_headers
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_currency_rate_invalid_code(self):
        ...
        response = self.client.post(
            "/currency/", {"currency_code": "Wrong"}, headers=self.auth_headers
        )
    ...
    def test_history_view(self):
        ...
        CurrencyExchange.objects.create(
            user=self.user, currency_code="USD", rate=40.0, created_at=now()
        )
